...
if __name__ == '__main__':
    ...
    dem_image_path_list = []
    path2indices = {}
    for image_index in range(len(dem_image_path_quad_list)):
        dem_image_path_quad = dem_image_path_quad_list[image_index]
        for quad_index in range(4):
            image_path = dem_image_path_quad[quad_index]
            dem_image_path_list.append(image_path)
            path2indices[image_path] = (image_index, quad_index, 'dem')

    eval_image_path_quad_list = eval_quad_data['image_path_quad']
    eval_image_path_list = []
    for image_index in range(len(eval_image_path_quad_list)):
        eval_image_path_quad = eval_image_path_quad_list[image_index]
        for quad_index in range(4):
            image_path = eval_image_path_quad[quad_index]
            eval_image_path_list.append(image_path)
            path2indices[image_path] = (image_index, quad_index, 'eval')

    image_path_list = dem_image_path_list + eval_image_path_list

    pdb.set_trace()
    print("\nstart running opensfm to get demonstration image locations\n")
    path2location = real_sfm.run_sfm(scene_name, image_path_list)
    print("\nsuccessfully running sfm and convert into {}/{} positions/paths\n".format(len(path2location), len(image_path_list)))
    for image_path in path2location.keys():
        print image_path
        print path2location[image_path]
    ...
    dem_position_list = []
    dem_angle_list = []
    dem_quad_index_list = [] # the quad index of this location to have sfm generated location
    for index in range(len(dem_image_path_quad_list)):
        # initialized as some false words
        dem_angle_list.append(None)
        dem_position_list.append(None)
        dem_quad_index_list.append(-1)

    eval_position_list = []
    eval_angle_list = []
    eval_quad_index_list = [] # the quad index of this location to have sfm generated location
    for index in range(len(eval_image_path_quad_list)):
        # initialized as some false words
        eval_angle_list.append(None)
        eval_position_list.append(None)
        eval_quad_index_list.append(-1)

    for image_path in path2location.keys():
        position_np, angle_np = path2location[image_path]
        image_index, quad_index, mode = path2indices[image_path]
        if mode == 'dem':
            last_best_quad_index = dem_quad_index_list[image_index]
            ...
            if record_location:
                dem_angle_list[image_index] = list(angle_np)
                dem_position_list[image_index] = list(position_np)
                dem_quad_index_list[image_index] = quad_index
        else:
            last_best_quad_index = eval_quad_index_list[image_index]
            record_location = False
            if last_best_quad_index == -1: # -1 (no previous recorded), then record
                record_location = True
            elif last_best_quad_index in [0,2]: # previous is front or back
            ...
            if record_location:
                eval_angle_list[image_index] = list(angle_np)
                eval_position_list[image_index] = list(position_np)
                eval_quad_index_list[image_index] = quad_index
    ...
    for index in range(len(dem_quad_index_list)):
        if dem_quad_index_list[index] != -1:
            # recordable
            position = dem_position_list[index]
            angle = dem_angle_list[index]
            time = -1
    ...
    image_count = 0
    for index in range(len(eval_quad_index_list)):
        if eval_quad_index_list[index] != -1:
            # recordable
            position = eval_position_list[index]
            angle = eval_angle_list[index]
